<template>
  <div class="flex flex-row relative w-full gap-2text-pink-500 rounded border border-solid border-zinc-500 border-opacity-70">
    <input ref="search" :value="modelValue" @input="onInputDebounce" @keydown.enter="enterPressed" type="search" :placeholder="placeholder" class="search-input rounded w-full py-2 pr-8 pl-2.5 text-xs font-medium text-right outline-none focus:outline-none">
